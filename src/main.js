...
const TEXT = {
    en: {
        ...
        legalDisclaimer: "Unofficial fan-made calculator. Not affiliated with, sponsored by, endorsed by, or approved by Niantic, Scopely, The Pokemon Company, Nintendo, Creatures, or GAME FREAK. Pokemon and related names are trademarks of their respective owners.",
        legalAccuracy: "This tool is informational only and depends on current game data and assumptions. Always verify important raid decisions in-game.",
        privacyLink: "Privacy",
        ...
        cpNotPossibleCopy: "No exact level 20 or level 25 IV spread can produce CP {cp} with the selected boss and IV floor.",
        ...
        possibleNoGood: "CP {cp} is possible, but none of its {total} IV spread{plural} purify to 100%. ",
    ...
    ja: {
        ...
        legalDisclaimer: "非公式のファンメイド計算ツールです。Niantic、Scopely、The Pokemon Company、Nintendo、Creatures、GAME FREAKとは提携・後援・承認関係にありません。Pokemonおよび関連名称は各権利者の商標です。",
        legalAccuracy: "このツールは参考情報です。ゲームデータや前提条件に依存するため、重要なレイド判断はゲーム内でも確認してください。",
        privacyLink: "プライバシー",
...
const elements = {
    pokemonInput: byId("pokemonInput"),
    pokemonOptions: byId("pokemonOptions"),
    pokemonSelect: byId("pokemonSelect"),
    pokemonHelp: byId("pokemonHelp"),
    ...
    languageStatus: byId("languageStatus"),
    themeButtons: Array.from(document.querySelectorAll("[data-theme-choice]")),
    languageButtons: Array.from(document.querySelectorAll("[data-language-choice]")),
...
function renderPokemonOptions() {
    ...
    elements.pokemonOptions.innerHTML = sortedPokemon().map((pokemon)=>`<option value="${escapeHtml(pokemonInputValue(pokemon))}" label="${escapeHtml(pokemon.name)}"></option>`).join("");
...
function restoreState() {
    ...
    prefs = {
        ...
        language: validateOption(saved.language, [
            "en",
            "ja"
...
function bindEvents() {
    ...
    elements.languageButtons.forEach((button)=>{
        button.addEventListener("click", ()=>{
            const nextLanguage = button.dataset.languageChoice;
...
function commitPokemonInput(forceMatch) {
...
function handleSummaryKeydown(event) {
    ...
    event.preventDefault();
    render();
    elements.contextStrip.querySelector(`[data-summary-control="${event.target.dataset.summaryControl}"]`)?.focus();
}
function syncPokemonInputToSelected() {
...
function renderStaticText() {
    ...
    document.querySelectorAll("[data-floor-custom]").forEach((option)=>{
...
function renderHundoHints(baseStats) {
    ...
    <span class="hundo-hint-values"><span>${copy("nonWeather")}</span> <strong>${maxCpFor(baseStats, RAID_LEVELS[0])}</strong></span>
    <span class="hundo-hint-values"><span>${copy("weatherBoosted")}</span> <strong>${maxCpFor(baseStats, RAID_LEVELS[1])}</strong></span>
  `;
}
function renderCpValidation(summaries, settings) {
    ...
    if (possible.length === 1) {
        ...
        elements.cpValidation.textContent = match.good ? formatCopy("validMatchGood", {
            scenario: scenarioLabel(match),
            good: match.good,
            total: match.total,
            odds: formatPercent(match.odds)
        }) : formatCopy("validMatchZero", {
            scenario: scenarioLabel(match)
        });
        return;
    }
    ...
    const message = inRange.length ? formatCopy("exactNoSpread", {
        cp: settings.cp
    }) : formatCopy("outsideRange", {
        ranges
    });
    elements.cpInput.setCustomValidity(message);
    elements.cpValidation.className = `field-help cp-validation ${possibleClass}`;
    elements.cpValidation.innerHTML = `${escapeHtml(message)}${nearest ? formatCopy("tryNearest", {
        chips: nearest
    }) : ""}`;
}
...
function renderAssumptionHints(settings) {
    ...
    elements.floorHint.textContent = formatCopy("floorHint", {
        floor: settings.raidFloor
    });
    elements.bonusHint.textContent = formatCopy("bonusHint", {
        bonus: settings.purifyBonus,
        target: threshold
    });
}
function renderPrimaryInsight(summaries, settings) {
    ...
    let body = formatCopy("cpNotPossibleCopy", {
        cp: settings.cp
    });
    if (best) {
        state = best.odds === 1 ? "good" : "mixed";
        title = best.odds === 1 ? copy("guaranteedChanceTitle") : formatCopy("mixedChanceTitle", {
            odds: formatPercent(best.odds)
        });
...
function renderResultPanel(summary) {
    ...
    <article class="result-panel is-${state.kind}" data-testid="${summary.raidLevel.key}-result">
      ...
      <div class="stat-row" aria-label="${formatCopy("watchTableLabel", {
        scenario: scenarioLabel(summary)
    })}">
        <div class="stat-item">
...
function renderPokemonSelectOptions(selectedName) {
    return sortedPokemon().map((pokemon)=>`<option value="${escapeHtml(pokemon.name)}" ${pokemon.name === selectedName ? "selected" : ""}>${escapeHtml(pokemonInputValue(pokemon))}</option>`).join("");
}
function renderIvFloorOptions(selectedFloor) {
    return Array.from({
        length: MAX_IV + 1
    }, (_value, floor)=>{
        const label = floor === 6 ? copy("floorShadow") : floor === 10 ? copy("floorStandard") : `${copy("floorCustom")}: ${floor}/${floor}/${floor}`;
        return `<option value="${floor}" ${floor === selectedFloor ? "selected" : ""}>${escapeHtml(label)}</option>`;
    }).join("");
}
function renderSummaryStats(baseStats) {
...
function resultState(summary) {
    if (!summary.total) {
        return {
            kind: "none",
            message: formatCopy("impossibleAtLevel", {
                cp: summary.cp,
                level: summary.raidLevel.level
            })
        };
    ...
    if (summary.good === summary.total) {
        return {
            kind: "good",
            message: formatCopy("guaranteedResult", {
                cp: summary.cp
            })
        };
    }
    return {
        kind: "mixed",
        message: formatCopy("mixedResult", {
            cp: summary.cp,
            total: summary.total,
            good: summary.good
        })
...
function renderComboPill(combo) {
    ...
    const title = formatCopy("comboTitle", {
        original,
        purified
    });
    return `
...
function renderWatchlistPanel(summary) {
    ...
    <details class="watch-panel" data-testid="${summary.raidLevel.key}-watchlist">
      <summary class="watch-head" data-watch-open="${escapeHtml(copy("watchOpen"))}" data-watch-close="${escapeHtml(copy("watchClose"))}">
        <div>
          <h2>${formatCopy("watchlistTitle", {
        scenario: scenarioLabel(summary)
    })}</h2>
      ...
      ${rows ? `<div class="watch-table" role="table" aria-label="${escapeHtml(formatCopy("watchTableLabel", {
        scenario: scenarioLabel(summary)
    }))}">
            ...
            </div>` : `<p class="empty-state">${copy("emptyWatchlist")}</p>`}
    </details>
  `;
}
...
function loadStateFromUrl() {
    ...
    if (pokemon && POKEMON.some((entry)=>pokemonNameMatches(entry, pokemon))) {
        state.selectedName = POKEMON.find((entry)=>pokemonNameMatches(entry, pokemon))?.name || pokemon;
    }
