...
const TEXT = {
    en: {
        ...
        possibleButZero: "{scenarios} can produce this CP, but none of the matching spreads purify to 15/15/15.",
        ...
        dataHint: "Seeded list: {count} bosses/forms, reviewed {date}. Use manual stats for brand-new or missing forms."
    },
    ja: {
        ...
        dataHint: "収録ボス/フォーム: {count}件。確認日: {date}。未収録や新フォームは種族値の手入力を使ってください。"
    }
};
const elements = {
...
function populatePokemonSelect() {
    ...
    elements.pokemonSelect.innerHTML = POKEMON.map((pokemon)=>`<option value="${escapeHtml(pokemon.name)}">${escapeHtml(pokemonDisplayName(pokemon))}</option>`).join("");
...
function bindEvents() {
    ...
    elements.languageButtons.forEach((button)=>{
        button.addEventListener("click", ()=>{
            prefs.language = button.dataset.languageChoice;
            render();
        });
    });
